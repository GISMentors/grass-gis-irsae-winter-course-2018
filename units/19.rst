...
.. code-block:: python
   ...
   # get list of sentinel band
   glist = Module("g.list", type="raster", patt="T32VNM_20180523T104019_B*_cor",
                  separator="comma", stdout_=PIPE)
   rasterlist = glist.outputs.stdout.split(',')
   # make a for loop for each Sentinel band
   for m in rasterlist:
       # set the right region 
       Module("g.region", raster=m)
       # convert the data
       Module("r.mapcalc", expression="{ma}_d=double({ma})".format(ma=m))
