...
Import all the DTM files downloaded from `hoydedata.no
...
type` and selecting :file:`geodata/lidar/dtm`.

.. figure:: ../images/units/17/import_dtm.png
   :class: middle

   Import all DTM TIF files from :file:`geodata/lidar/dtm/` directory.
...
In the last step difference beetween the imported DTM and interpolated
DTM is computed using :grasscmd:`r.mapcalc`.
...
.. figure:: ../images/units/17/chm.png
   ...
   The CHM map (difference between interpolated and imported DTM).
